...
if __name__ == '__main__':
    ...
    while ( (NV<M-1-NV_LIM) and (itr<MAXITR) ) :
        print('===================================================')
        print('>>> FINDING -', NV+1,'TH VECTOR OF -',M, 'ITER=', itr+1, 'of', MAXITR)
        print('===================================================')

        # calculate sweep numbers
        NTSWEEPS=updateMAXREADS(NSWEEPS, NV)
        # current status
        #print('CLQ(G):', len(xclq.max_clique(G)), ', |G|=', len(G.nodes()))
        prevNDG=len(G.nodes())
        prevNDSG=len(SG.nodes())
        prevMDG=mean_degree(G)
        prevMDSG=mean_degree(SG)
        # update list for curve display
        vLenG.append(prevNDG)
        vLenSG.append(prevNDSG)
        vMeanLenG.append(prevMDG)
        vMeanLenSG.append(prevMDSG)
        #
        '''
        #********************************************************************
        ...
        vParams=requestJobs(JOB_LIST, M, NJOBS, NTSWEEPS, NREADS)
        updateTRIAL(TRIAL, vParams)        
        # --------------------------------------------------------------------
        # spwan jobs and get results
        # --------------------------------------------------------------------
        #execute parallel jobs, then update G
        tV1=pool.map(findOrthoVec_PAR, vParams)
        G, CLQ = updateGRAPH(tV1, G, CLQ, MAXBRANCH)
        '''
        #********************************************************************
        ...
        postNDG=len(G.nodes())
        postMDG=mean_degree(G)        
        print('#Nodes in G:', prevNDG, '->', postNDG, \
              '; Mean degree:',int(prevMDG), '->', int(postMDG))
        # --
        ...
        NV=len(CLQ)
        '''
        # CONSTRUCT DENSE SUBGRAPH
        '''
        minOCLQ=int(3*NV/4) #int(3*NV/4) #NV*2/3
        if NV>M/4:
            ...
            SG=G.copy()
        # ----
        postNDSG=len(SG.nodes())
        postMDSG=mean_degree(SG)
        print('#Nodes in SG:', prevNDSG, '->', postNDSG, \
              '; Mean degree:',int(prevMDSG), '->', int(postMDSG))
        # --- memory size
